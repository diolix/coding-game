...
public abstract class BaseOutputValueHandle(string label, ValueTypeEnum valueTypeEnum, INode node)
    : BaseValueHandle(label, valueTypeEnum, node)
{
    ...
    protected override bool IsCompatible(IHandle handle) => handle is InputValueHandle inputValueHandle && (
        inputValueHandle.IsOfType(ValueTypeEnum) ||
        inputValueHandle.IsOfType(ValueTypeEnum.Object));
